<template>
  <PaymentSettingsLayout>
    <div class="flex flex-wrap items-end justify-between gap-3"><div><h2 class="text-xl font-semibold tracking-normal">支付日志</h2><p class="mt-1 text-sm text-muted-foreground">仅展示已脱敏的支付事件与验证结果。</p></div><div class="flex flex-wrap items-center gap-2"><Button variant="outline" size="sm" :disabled="loading || clearing" aria-label="刷新" title="刷新" @click="loadLogs"><RefreshCwIcon :class="loading ? 'animate-spin' : ''" />刷新</Button><Button variant="outline" size="sm" class="border-destructive/30 text-destructive hover:border-destructive/50 hover:bg-destructive/10 hover:text-destructive" :disabled="loading || clearing || logTotal === 0" @click="clearDialogOpen = true"><Trash2Icon />清空日志</Button></div></div>
    <Card class="mt-4"><CardContent class="grid gap-4 pt-6"><AdminDataTable :columns="logColumns" :rows="logs" row-key="id" empty-text="暂无支付日志。"><template #toolbar><div class="flex flex-wrap items-center gap-2"><Select v-model="draftLogProvider"><SelectTrigger size="sm" class="w-40 shrink-0"><SelectValue placeholder="全部渠道" /></SelectTrigger><SelectContent><SelectItem value="ALL">全部渠道</SelectItem><SelectItem v-for="item in providers" :key="item.provider" :value="item.provider">{{ item.title }}</SelectItem></SelectContent></Select><Button size="sm" :disabled="loading || clearing" @click="search">查询</Button><Button variant="outline" size="sm" :disabled="loading || clearing" @click="resetFilters">重置</Button></div></template><template #cell-orderNo="{ value }"><span class="font-mono text-xs">{{ value || "-" }}</span></template><template #cell-createdAt="{ value }"><span class="text-xs">{{ formatDate(value) }}</span></template><template #pagination><Pagination v-model:page="logPage" v-model:page-size="logPageSize" :total="logTotal" /></template></AdminDataTable></CardContent></Card>

    <Dialog v-model:open="clearDialogOpen"><DialogContent class="max-w-md"><DialogHeader class="pr-8"><DialogTitle>清空全部支付日志？</DialogTitle><DialogDescription>将永久删除全部支付日志，不受当前渠道筛选影响。此操作不可恢复。</DialogDescription></DialogHeader><DialogFooter><DialogClose as-child><Button variant="outline" :disabled="clearing">取消</Button></DialogClose><Button variant="destructive" :disabled="clearing" @click="clearLogs">{{ clearing ? "清空中..." : "确认清空" }}</Button></DialogFooter></DialogContent></Dialog>
  </PaymentSettingsLayout>
</template>
...
import { onClearPaymentLogs, onGetPaymentLogs, onGetPaymentProviders } from "@/server/payment/admin.telefunc";
...
const providers = ref<Provider[]>([]); const logs = ref<PaymentLog[]>([]); const draftLogProvider = ref<"ALL" | PaymentProviderKind>("ALL"); const logProvider = ref<"ALL" | PaymentProviderKind>("ALL"); const logPage = ref(1); const logPageSize = ref(10); const logTotal = ref(0); const loading = ref(false); const clearing = ref(false); const clearDialogOpen = ref(false);
...
async function clearLogs() { clearing.value = true; try { await runTelefunc(() => onClearPaymentLogs(), { successMessage: "支付日志已清空。" }); clearDialogOpen.value = false; logPage.value = 1; await loadLogs(); } catch { /* runTelefunc owns feedback */ } finally { clearing.value = false; } }
